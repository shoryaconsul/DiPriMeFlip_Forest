...
# This file implements the proposed algorithm for classifcation (stores fractions).
# The variables mean refer to fractions in this code.
...
# Select index using permute-and-flip mechanism
# score: Scores for each index
# sens: Sensitivity of scoring function
# eps: Epsilon (privacy parameter)
def pf_mech(score, sens, eps):
    score_max = np.amax(score)  # Max score
    prob = np.exp(0.5*eps*(score-score_max)/sens) + 1e-12
    while True:
        ind_perm = rn.permutation(np.arange(len(score)))  # Permuted indices
        ind_flip = rn.rand(len(prob)) < prob[ind_perm]
        ind_sel = np.where(np.cumsum(ind_flip) == 1)[0]
    # while True:  # Keep sampling until valid sample found
    #     ind_flip = np.nonzero(rn.rand(len(prob)) < prob)[0]
    #     if len(ind_flip) > 0:  # If sample found
...
# Function to find best split for selected attributes
# X: Data (M,N)
# y: Target (M,)
...
def split_count(X, y, A, By, cat_idx, K, eps=None):
    ...
    # Exponential mechanism to pick split
        if len(y) != 0:
            idx_pick = pf_mech(-1*sse_idx, 4*By**2/len(y), eps_fn)
            # prob_split = np.exp(-0.5*sse_idx*eps/(4*By**2/len(y)))
        else:  # Empty node, so all splits are equivalent
            idx_pick = pf_mech(np.ones(K1), 1, eps_fn)  # Random pick
            # prob_split = np.ones(K1)
        # print('# SPLIT: ',len(y_upp),len(y_low))
        # print('SSE ',sse_idx,' SCORE ',score_split)
        # idx_split = rn.choice(idx_cand, p=prob_split/np.sum(prob_split))
        idx_split = idx_cand[idx_pick]
    return idx_split, val_cand[idx_split]


# %% Base classes for Extremely Random Trees with splits along median

class DiPrimeFlipCTree():
    ...
    # cat_idx: List of indices for categorical features
    # tbound: Class values. If single value given then classes assumed to be [-tbound, tbound]
    ...
    def fit(self, X, y, A, cat_idx, tbound, eps=None, b_med=0.5):
        ...
        tree_upp = DiPrimeFlipCTree(depth=self.depth+1, max_depth=self.max_depth,
                                max_features=self.max_features, parent=self)
        tree_upp.fit(X_upp, y_upp, A_upp, cat_idx, tbound, eps, b_med)
        self.left = tree_upp

        tree_low = DiPrimeFlipCTree(depth=self.depth+1, max_depth=self.max_depth,
                                max_features=self.max_features, parent=self)
...
class DiPrimeFlipCForest():
    def __init__(self, n_trees=10, max_depth=np.inf, max_features=None,
        ...
        for i in range(n_trees):  # Initialize all trees
            self.trees.append(DiPrimeFlipCTree(
                depth=0, max_depth=max_depth, max_features=max_features,
                parent=None))
