...
def query_cont_med(Xj, Xrange, eps=None):
    ...
    if M <= 1:
        ...
        score = -1*np.abs(np.arange(-M, M+1, 2)
                          )  # Scoring function is -|N_L-N_R|
...
class DiPrimeFlipTree():
    ...
    def fit(self, X, y, A, cat_idx, tbound, eps=None, b_med=0.5):
        ...
        tree_upp = DiPrimeFlipTree(depth=self.depth+1, max_depth=self.max_depth,
                             max_features=self.max_features, parent=self)
        tree_upp.fit(X_upp, y_upp, A_upp, cat_idx, tbound, eps, b_med)
        self.left = tree_upp

        tree_low = DiPrimeFlipTree(depth=self.depth+1, max_depth=self.max_depth,
                             max_features=self.max_features, parent=self)
...
class DiPrimeFlipForest():
    def __init__(self, n_trees=10, max_depth=np.inf, max_features=None,
        ...
        for i in range(n_trees):  # Initialize all trees
            self.trees.append(DiPrimeFlipTree(depth=0, max_depth=max_depth,
